...
class RNNPredictor(nn.Module):
    ...
    def __init__(self, rnn_type, enc_inp_size, rnn_inp_size, rnn_hid_size, dec_out_size, nlayers, dropout=0.5,
        ...
        middelDims.insert(0,enc_inp_size)
        middelDims.append(rnn_inp_size)
        FCDims = middelDims
        ordereddict = collections.OrderedDict()
        for i in range(len(FCDims)-1):
            ordereddict['linear'+str(i)] = nn.Linear(FCDims[i],FCDims[i+1])
        ...
        ordereddict_de = collections.OrderedDict()
        for i in range(len(FCDims)-2,-1,-1):
            ordereddict_de['linear'+str(i)] = nn.Linear(FCDims[i+1],FCDims[i])

        self.decoder = nn.Sequential(ordereddict_de)
