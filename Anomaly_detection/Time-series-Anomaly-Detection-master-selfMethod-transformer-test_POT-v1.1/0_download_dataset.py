    ...
    for filepath in raw_dir.glob('*.txt'):
        with open(str(filepath)) as f:
            ...
            for i, line in enumerate(f):
                ...
                if raw_dir.parent.name== 'ecg':  # 这个应该是说，第一个维度是时间
                    # Remove time-step channel
                    tokens.pop(0)
                if filepath.name == 'chfdbchf15.txt':  # 这底下的代码是人工标注数据集的时候
                                                       # 能看出来，基本也是一个机器一段异常
            ...
            if filepath.name == 'chfdb_chf13_45590.txt':  # 底下也是人工算好数据长度，然后分别保存的
