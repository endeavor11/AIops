...
scores_transformer_train = anomalyScore_transformer(args, model, train_dataset, means, covs) # [allLen - bptt, feature_dim]
supplement = torch.zeros((args.bptt, nfeatures)).cuda()
scores_transformer_train = torch.cat((supplement, scores_transformer_train), dim=0)
...
try:
    ...
    for channel_idx in range(nfeatures):
        ...
        train_score = scores_transformer_train[:,channel_idx]
        train_score[train_score<0.0] = 0.0
        ...
        trainScoreList.append(train_score.cpu().numpy())
    ...
    trainScoreList = np.array(trainScoreList)

    testScoreList = np.sum(testScoreList, axis=0)
    trainScoreList = np.sum(trainScoreList, axis=0)
    ...
    print(code_zsx.eval_methods.pot_eval(-1 * trainScoreList, -1 * testScoreList, TimeseriesData.testLabel.cpu().numpy()))
