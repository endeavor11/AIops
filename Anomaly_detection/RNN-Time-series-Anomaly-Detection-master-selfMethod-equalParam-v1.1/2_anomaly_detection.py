...
from anomalyDetector import get_precision_recall_zsx
...
model = modelOld.RNNPredictor(rnn_type = args.model,
...
try:
    ...
    for channel_idx in range(nfeatures):
        ...
        if 'means' in checkpoint.keys() and 'covs' in checkpoint.keys():
            ...
            mean, cov = fit_norm_distribution_param(args, model, train_dataset, channel_idx=channel_idx) # 具体意义可OneNote
        ...
        precision, recall, f_beta = get_precision_recall_zsx(args, score, num_samples=1000, beta=args.beta,
                                                         label=TimeseriesData.testLabel.to(args.device))
        ...
        print('data: ',args.data,' filename: ',args.filename,
              ' f-beta (no compensation): ', f_beta.max().item(),' beta: ',args.beta)  # 取最大的一个f-beta！
        if args.compensate:
            precision, recall, f_beta = get_precision_recall(args, score, num_samples=1000, beta=args.beta,
                                                             label=TimeseriesData.testLabel.to(args.device),
        ...
        target = preprocess_data.reconstruct(test_dataset.cpu()[:, 0, channel_idx],  # 普通调用这个重建函数
        ...
        mean_prediction = preprocess_data.reconstruct(sorted_prediction.mean(dim=1).cpu(),
                                                      ...
                                                      TimeseriesData.std[channel_idx]).numpy()
                                # sorted_prediction  [time_length,10] 每个时刻的10个预测值
                                # mean(dim=1)后，每个时刻只有一个预测值了
        
        oneStep_prediction = preprocess_data.reconstruct(sorted_prediction[:, -1].cpu(),
                                                         TimeseriesData.mean[channel_idx],
                                                         TimeseriesData.std[channel_idx]).numpy()
                                # 这个取的是每个时刻的最后一个预测值，因为预测值取的顺序是
                                # 从左往右，所以，这个index最后的这个值，就是一步预测出来的！
        
        Nstep_prediction = preprocess_data.reconstruct(sorted_prediction[:, 0].cpu(),
                                                       TimeseriesData.mean[channel_idx],
                                                       TimeseriesData.std[channel_idx]).numpy()
                                # 隔得最远的预测出来的结果，肯定是效果最差的
        # 上面几个预测的shape [length,] 因为只有一个维度
        
        sorted_errors_mean = sorted_error.abs().mean(dim=1).cpu()
                                # sorted_error [time_length,10]  每个时刻的预测误差
                                # sorted_error_mean [length,]
        sorted_errors_mean *= TimeseriesData.std[channel_idx]  # 乘标准差转换
        sorted_errors_mean = sorted_errors_mean.numpy()
        score = score.cpu()  # score [length,]  target [length,] prediction_score 空数组
        ...
        # mean_prediction [time_length] 每个时刻的平均预测值
        # oneStep_prediction [length,] 每个时刻的一步预测值，某个特征通道
        # Nstep_prediction 类似
        ...
        if args.save_fig:
            ...
            plt.plot(precision.cpu().numpy(),label='precision')  # 画一个列表，精度随th的变化
