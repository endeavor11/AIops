...
def generate_output(args,epoch, model, gen_dataset, disp_uncertainty=True,startPoint=500, endPoint=3500):
    # 整个函数，只有save-fig是true的时候才运行代码
    """
    gen_dataset                     :普通的测试训练集，batchsize=1得到的
                                    :[time,1,feature_dim]
    ...
    if args.save_fig:
        ...
        with torch.no_grad():
            for i in range(endPoint):
                ...
                if i>=startPoint:
                    ...
                    # 如果i还不到startpoint
                    # for这段代码，会先执行这部分，注意，从这里需要看出seq的值！！！
                    out, hidden = model.forward(gen_dataset[i].unsqueeze(0), hidden)
                    # 如果batchsize为1的话，那么out的形状
                    # out size: [seq,1,hidden_size] 
                    # 注意这个seq有可能是1，反正只去了seq序列的第一个输出
                    
                    # 明确！
                    # gen_dataset[i].unsqueeze(0) shape [1,1,feature_dim]
                    # out [1,1,feature_dim]
                    # 喂进去的 是 seq=1 的数据！
                outSeq.append(out.data.cpu()[0][0].unsqueeze(0))
                    # append 之前 [hidden_size]
                    # unsqueeze后 [1,hidden_size]
                    # 经过上面这个for 把start和end之间的所有预测值都拿了一遍


        outSeq = torch.cat(outSeq,dim=0) # [seqLength * feature_dim]
                                         # 实际上是 [seq,feature_dim]
                                         # 长度是 endpoint-startpoint
        ...
        # 注意这个target 和 outseq 估计就是最后的结果了！差值可以作为损失函数的那个

        plt.figure(figsize=(15,5))
        for i in range(target.size(-1)):  # 每个维度 target size [seq,feature_dim]
            ...
            # 上面这个之所以叫 一部预测，就是因为 喂进去的都是真值！只有一个输出，这个输出是下一步的预测
            ...
            # 上面这个叫迭代预测！因为是依靠上一步RNN的输入当做输入来得出新的输出的！
...
def train(args, model, train_dataset,epoch):
    ...
    with torch.enable_grad():
        ...
        for batch, i in enumerate(range(0, train_dataset.size(0) - 1, args.bptt)):
            ...
            if batch % args.log_interval == 0 and batch > 0:  # 训练一些后进行输出
