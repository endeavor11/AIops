...
scheduler_model_server_0 = MultiStepLR(optimizer_model_server_0, milestones=args.milestones, gamma=args.gamma)
scheduler_encoder_client_0 = MultiStepLR(optimizer_encoder_client_0, milestones=args.milestones, gamma=args.gamma)
scheduler_encoder_client_1 = MultiStepLR(optimizer_encoder_client_1, milestones=args.milestones, gamma=args.gamma)
scheduler_encoder_client_2 = MultiStepLR(optimizer_encoder_client_2, milestones=args.milestones, gamma=args.gamma)
scheduler_decoder_client_0 = MultiStepLR(optimizer_decoder_client_0, milestones=args.milestones, gamma=args.gamma)
scheduler_decoder_client_1 = MultiStepLR(optimizer_decoder_client_1, milestones=args.milestones, gamma=args.gamma)
scheduler_decoder_client_2 = MultiStepLR(optimizer_decoder_client_2, milestones=args.milestones, gamma=args.gamma)
...
def train(args, model_server_0,encoder_client_0,encoder_client_1,encoder_client_2,
    ...
    with torch.enable_grad():
        ...
        scheduler_model_server_0.step()
        scheduler_encoder_client_0.step()
        scheduler_encoder_client_1.step()
        scheduler_encoder_client_2.step()

        scheduler_decoder_client_0.step()
        scheduler_decoder_client_1.step()
        scheduler_decoder_client_2.step()
...
if not args.pretrained:  # 模型训练 默认False ，需要训练，就执行这个分支
    ...
    try:
        for epoch in range(start_epoch, args.epochs+1):
            ...
            winName = args.filename.split(".")[0]+"_FL" + str(args.index) + str(args.index)
...
means, covs = fit_norm_distribution_param_transformer(args, model_server_0,encoder_client_0,encoder_client_1,encoder_client_2,
                                                     decoder_client_0,decoder_client_1,decoder_client_2,
