...
def data2rightKPI(data,dimSeg,length=3000,allLength=15000,beginIndex=0,interval=10,smoothLength=101):
    pearsonsArrList = getselfPearson(data,length=3000,allLength=15000,beginIndex=0,interval=10,smoothLength=101)
    KPIs = pickRightKPI(data,pearsonsArrList,dimSeg)

    return KPIs


def getShiftResult(data,KPIS,interval=10,minShift=-1000,maxShift=1000,beginIndex=1000,T=1500):
    """
    ...
    for i in range(1,len(KPIS)):
        shiftResult.append(getShift(data[:,KPIS[0]],data[:,KPIS[i]]))
